<div id="comment-{{ $comment->comment_id }}"
     class="chat-message {{ $comment->user_id === auth()->id() ? 'self' : 'other' }} {{ $level > 0 ? 'reply-box' : '' }}">
  <div class="message-bubble" style="
        background: {{ $comment->user_id === auth()->id() ? '#dcf8c6' : '#ffffff' }};
    ...
    <form id="reply-form-{{ $comment->comment_id }}" class="reply-form mt-2 d-none"
          action="{{ route('comments.card.store', $comment->card_id) }}" method="POST">
      @csrf
