...
<head>
  ...
  <title>Dashboard Designer - Project PRO</title>
  ...
  <style>
    ...
    .logo-circle i { font-size: 1.9rem; color: var(--primary); }
    ...
    .card-header .badge { background: rgba(255,255,255,0.3); }
    ...
    .btn-outline-danger-modern {
      background: #ef4444;         /* tombol merah solid */
      color: #fff;
      font-weight: 600;
      border: none;
      border-radius: 8px;
    ...
    .btn-outline-danger-modern:hover {
      background: #f53117ff;         /* hover oranye */
      color: #fff;
    ...
    .btn-success.btn-action {
      ...
      font-weight: 600;
      border-radius: 7px;         /* sedikit lebih ramping */
      padding: 7px 0;             /* tinggi tombol lebih kecil */
      font-size: 0.95rem;         /* teks sedikit diperkecil */
      transition: 0.3s;
    ...
    .btn-success:hover { background: var(--success-gradient-hover); }
   ...
   .table {
    ...
    tbody tr:hover { background: #f1f5f9; transition: 0.2s; }
    ...
    .alert-danger-modern {
      background: #fee2e2; /* lembut merah */
      color: #991b1b;
    }

    .alert-success-modern {
      background: #dcfce7; /* lembut hijau */
      color: #166534;
    }
      /* === RESPONSIVE === */
...
<body>
  ...
  <div class="sidebar">
    <div>
      <div class="text-center">
        <div class="logo-circle"><i class="bi bi-kanban"></i></div>
        <h4>Project PRO</h4>
      </div>
      <a href="{{ route('designer.dashboard') }}" class="nav-link active"><i class="bi bi-speedometer2"></i> Dashboard</a>
      <a href="{{ route('designer.myteam') }}" class="nav-link"><i class="bi bi-people-fill"></i> Tim Saya</a>
    ...
    <form action="{{ route('logout') }}" method="POST">@csrf
      <button type="submit" class="logout-btn w-100"><i class="bi bi-box-arrow-right"></i> Logout</button>
    </form>
  </div>
  ...
  <div class="content">
    <div class="page-header">
      ...
      <h2><i class="bi bi-brush"></i> Desain Anda</h2>

      <!-- SEARCH -->
      <div class="search-container">
        <form method="GET" id="search-form" action="{{ route('designer.dashboard') }}" class="search-box">
    ...
    @if($filteredCards->isEmpty())
      <div class="alert alert-info shadow-sm">Belum ada tugas desain untuk Anda.</div>
    @else
      @foreach($filteredCards as $card)
        <div class="card mb-4">
          ...
          <div class="card-body">
            ...
            <p><strong>Status Desain:</strong> {{ ucfirst($card->status) }}</p>
                ...
                <tbody>
                  @forelse($card->subtasks->sortBy('position') as $subtask)
                    <tr>
                      ...
                      <td>
                        @if($subtask->status == 'todo')
                          <form action="{{ route('subtasks.start', $subtask->subtask_id) }}" method="POST" class="mb-2">@csrf
                           <button type="submit" class="btn btn-primary wide">🎨 Mulai</button>
                          </form>
                          ...
                          <form action="{{ route('subtasks.complete', $subtask->subtask_id) }}" method="POST">@csrf
                              <button type="submit" class="btn btn-success btn-action">✅ Selesai</button>
                          </form>
                      ...
                      <td><a href="{{ route('comments.subtask.index', $subtask->subtask_id) }}" class="btn btn-comment">💬 Komentar</a></td>
                    </tr>
                  @empty
                    <tr><td colspan="7" class="text-muted text-center">Belum ada tahapan desain</td></tr>
                  @endforelse
                </tbody>
              </table>
            </div>
